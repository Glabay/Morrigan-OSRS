...
object Fishing {
    ...
    private fun canFish(p: Player, npc: FishingOption): Boolean {
        ...
        if (tool == null) {
            p.filterableMessage("You need a ${p.world.definitions.get(ItemDef::class.java, npc.toolId).name} to fish here.")
            return false
        }
        ...
        if (npc.bait != null) {
            ...
            if (bait == null) {
                p.filterableMessage("You need ${p.world.definitions.get(ItemDef::class.java, npc.bait).name} to bait this fish.")
                return false
            }
        }
        if (p.getSkills().getMaxLevel(Skills.FISHING) < npc.levelReq) {
            p.filterableMessage("You need a Fishing level of ${npc.levelReq} to fish here.")
            return false
        }

        if (p.inventory.isFull) {
            p.filterableMessage("Your inventory is too full to hold any more fish.")
            return false
        }
